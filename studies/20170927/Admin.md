...
- Form field의 queryset을 설정해보자.  
	스페셜 토핑을 선택하기 위해서, 모든 토핑 모델의 데이터를 선택지에 올릴 필요는 없다. 
	...
	class IceCreamForm(forms.ModelForm):
		...
		def __init__(self, *args, **kwargs):
		super().__init__(*args, **kwargs)
		self.fields['special_topping'] = forms.ModelChoiceField(queryset=Topping.objects.filter(category='special'), label='스페셜 토핑', required=False)
...
class IceCreamAdmin(admin.ModelAdmin):
	...
	def has_delete_permission(self, request):
	    return False
```
기본 permission을 False로 설정하고 action으로 재정의 할 수 있다.   

특정 객체만 바로 수정, 삭제하는 경우에 문제가 되는 케이스에 이용할 수 있음.  
재정의한 action에서 연관된 데이터를 처리할 수 있다.

## 하나의 모델에 대해 두 개의 어드민 페이지를 만들 수 있을까?
model은 admin resister는 한번만 가능.
