...
- 템플릿 레이어에서의 쿼리 수행과 이터레이션은 특히 문제가 된다.
 ...
  - 각 이터레이션 루프 때마다 얼마나 많은 프로세싱이 벌어지는가?
- 캐시도 괜찮지만 그 전에 원인을 먼저 분석해 고쳐보자.

- 지금부터 피해야 할 몇 가지 주의사항을 살펴보자.
...
- 간단한 템플릿 코드라도 상당한 프로세싱을 필요로 하는 객체가 호출되어 상당한 부하를 일으킬 수 있다.
- 많은 양의 이미지나 데이터를 처리하는 프로젝트는 뷰나 모델, 헬퍼 메서드, 또는 셀러리 등을 이용한 비동기 메시지 큐 시스템으로 처리해야된다.
...
- base.html
  - title : TWO Scoops of Django
  ...
  - 템플릿 태그들이 이용되어 있다.
여기서 커스텀 타이틀, 기본 스타일시트와 추가 스타일시트, 기본 헤더, 서브헤더, 본문, 자식블록의 이용, 
...
- 이용한다면 html파일을 확장하여 이용하면 {{ block.super}}를 이용해 템플릿들을 관리할 수 있다.
...
- 암묵적인 {{object}}, {{object_list}} 대신에 명시적인 {topping_list}}, {{toppin}}을 이용할 수 있다.
...
- TEMPLATES 세팅의 OPTIONS에서 string_if_invalid 옵션을 설정함으로써 좀 더 자세한 에러 메세지를 받아 볼 수 있다.
